...
function LogoThemedImage({ logo, alt, imageClassName }: { logo: NavbarLogo; alt: string; imageClassName?: string }) {
  ...
  return imageClassName ? <div className={cn(imageClassName, 'mr-0')}>{themedImage}</div> : themedImage;
}
...
export default function Logo(props: Props): JSX.Element {
  ...
  return (
    <>
      ...
      <Link to="/enterprise" {...propsRest} {...(logo?.target && { target: logo.target })} className={cn(propsRest.className, "sm:hidden")}>
